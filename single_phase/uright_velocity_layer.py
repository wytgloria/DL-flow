...
class URightLayer(Layer):
  ...
  def build(self, input_shape):
    ...
    self.W2 = K.variable(weight_init1)
    self.b = K.zeros((self.input_dim,))
    self.trainable_weights = [self.W2, self.b]
  ...
  def call(self, inputs, mask=None):
    ...
    sparse_w2 = tf.SparseTensor(indices=ind, values=self.W2, dense_shape=[self.input_dim, self.input_dim])

    sp_w2 = sparse_w2.__mul__(velocity)
    result = tf.transpose(tf.sparse_tensor_dense_matmul(sp_w2, tf.transpose(input_sat)))+ self.b
  ...
  def get_config(self):
    ...
    base_config = super(URightLayer, self).get_config()
