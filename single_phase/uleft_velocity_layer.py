...
class ULeftLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    velocity = 1250*velocity[0, :]
    velocity = tf.constant(velocity, dtype=tf.float32, shape=[self.input_dim, ])
    ...
    sp_w1 = sparse_w1.__mul__(velocity)
    result = tf.transpose(tf.sparse_tensor_dense_matmul(sp_w1, tf.transpose(input_sat)))+ self.b
    return result
  ...
  def get_config(self):
    ...
    base_config = super(ULeftLayer, self).get_config()
