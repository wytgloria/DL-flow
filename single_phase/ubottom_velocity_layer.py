...
class UBottomLayer(Layer):
  ...
  def build(self, input_shape):
    ...
    self.W4 = K.variable(weight_init1)
    self.b = K.zeros((self.input_dim,))
    self.trainable_weights = [self.W4, self.b]
  ...
  def call(self, inputs, mask=None):
    ...
    sparse_w4 = tf.SparseTensor(indices=ind, values=self.W4, dense_shape=[self.input_dim, self.input_dim])

    sp_w4 = sparse_w4.__mul__(velocity)
    result = tf.transpose(tf.sparse_tensor_dense_matmul(sp_w4, tf.transpose(input_sat)))+ self.b
  ...
  def get_config(self):
    ...
    base_config = super(UBottomLayer, self).get_config()
